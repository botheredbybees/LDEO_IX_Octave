...
| `magdec/` | `magdec.py`, the real `magdec` CLI replacement installed into the Docker image (see `CHANGES.md`'s "External tools added"), backed by `ppigrf` (IGRF-14). Tests are `magdec/test_magdec.py`, run via `python -m pytest magdec/test_magdec.py` — note `ppigrf` isn't in `webapp/requirements.txt`, so a `.venv312` provisioned from that file alone will fail these tests; also install `magdec/requirements.txt` (`pip install -r magdec/requirements.txt`). |
